...
<body>
	...
	<form:form id="searchForm" modelAttribute="scoAcceptanceReport" action="${ctx}/sco/scoAcceptanceReport/" method="post" class="breadcrumb form-search">
					...
					<c:if test="${fns:getDictValue('审核通过', 'audit_report', '2') == scoAcceptanceReport.state
					 || fns:getDictValue('默认通过', 'audit_report', '3') == scoAcceptanceReport.state }">
						 ...
						 <c:if test="${param.type == '3'}">
							<a href="${ctx}/sco/scoAcceptanceReport/report/print?id=${scoAcceptanceReport.id}&type=${param.type}" target="_blank">打印</a>
						</c:if>
						<c:if test="${param.type == '5' || param.type == '6'}">
							<a href="${ctx}/sco/scoAcceptanceReport/report/print/print?id=${scoAcceptanceReport.id}&type=${param.type}" target="_blank">打印</a>
						</c:if>
<%-- 					<a href="${ctx}/sco/scoAcceptanceReport/report/print?id=${scoAcceptanceReport.id}" target="_blank">打印</a> --%>
					</c:if>
					
					 <c:if test="${param.type == '3'}">
						<a href="${ctx}/sco/scoAcceptanceReport/report/view?id=${scoAcceptanceReport.id}&type=${param.type}" target="_blank">查看</a>
<%-- 						<a href="${ctx}/sco/scoAcceptanceReport/report/view?id=${scoAcceptanceReport.id}" target="_blank">查看</a> --%>
					</c:if>
					<c:if test="${param.type == '5' || param.type == '6'}">
