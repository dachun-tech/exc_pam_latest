...
<head>
	...
	<script type="text/javascript">
		...
		function checkTotalAmt(totalAmt, href){
			if(totalAmt > 50000.00){
				alertx('总金额不能大于50000.00元,请调整后提交!');
				return false;
			}
...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="scoGoodsAcceptance">
			<tr>
				<td>
					${scoGoodsAcceptance.goodsId.name}
				</td>
				<td>
					<fmt:formatNumber value="${scoGoodsAcceptance.goodsId.normalPrice}" type="currency" pattern="￥0.00"/>
				</td>
				<td>
					<fmt:formatNumber value="${scoGoodsAcceptance.goodsId.agrtPrice}" type="currency" pattern="￥0.00"/>
				</td>
				<td>
					<fmt:formatNumber value="${scoGoodsAcceptance.discountPercent}" type="percent"/>
				</td>
				<td>
					${scoGoodsAcceptance.goodsId.units}
				</td>
				...
				<shiro:hasPermission name="sco:scoGoodsAcceptance:edit"><td>
					<a href="${ctx}/sco/scoGoodsAcceptance/delete?id=${scoGoodsAcceptance.id}" onclick="return confirmx('确认要删除该商品吗？', this.href)">删除</a>
	...
	<shiro:hasPermission name="sco:scoGoodsAcceptance:edit">
		<div style="padding-top: 20px;padding-left: 10px;">
			<a href="${ctx}/sco/scoGoods/list?arId=${scoGoodsAcceptance.id}&handle=addGoods&type=${param.type}">添加商品到验收单</a>&nbsp;&nbsp;
			<a href="${ctx}/sco/scoAcceptanceReport/addGoodsFinish?id=${scoAcceptanceReport.id}&type=${param.type}" onclick="return checkTotalAmt('${totalAmt}', this.href)">录入完毕</a>&nbsp;&nbsp;
			<a href="${ctx}/sco/scoAcceptanceReport/report/view?id=${scoAcceptanceReport.id}&type=${param.type}" target="_blank">打印预览</a>&nbsp;&nbsp;
