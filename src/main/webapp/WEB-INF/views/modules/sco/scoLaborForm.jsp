...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/sco/scoLabor/list?type=${param.type}">${fns:getDictValueBySuffix("lable_title", "second_form_", param.type, "")}列表</a></li>
		<li class="active"><a href="${ctx}/sco/scoLabor/form?id=${scoLabor.id}&type=${param.type}">${fns:getDictValueBySuffix("lable_title", "second_form_", param.type, "")}<shiro:hasPermission name="sco:scoLabor:edit">${not empty scoLabor.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sco:scoLabor:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="scoLabor" action="${ctx}/sco/scoLabor/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="serTreeId" class="input-medium required" onchange="serSubTree(this.value);">
				...
				<form:select path="subId" class="input-medium required">
		...
		<c:if test="${fns:getDictValueBySuffix('lable_commitment', 'second_form_', param.type, '') != ''}">
			<div class="control-group">
				...
				<div class="controls">
					<form:textarea path="commitment" htmlEscape="false" rows="3" maxlength="500" class="input-xlarge"/>
				</div>
			</div>
		</c:if>
		...
		<div class="form-actions">
			<shiro:hasPermission name="sco:scoLabor:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
